...
internal_int_prime_is_divisible :: proc(a: ^Int, allocator := context.allocator) -> (res: bool, err: Error) {
	...
	internal_clear_if_uninitialized(a) or return

	for prime in _private_prime_table {
		rem := #force_inline int_mod_digit(a, prime) or return
		if rem == 0 {
			return true, nil
		}
	}
...
internal_int_exponent_mod :: proc(res, G, X, P: ^Int, allocator := context.allocator) -> (err: Error) {
	...
	*/
	if internal_is_negative(X) {
		...
		internal_init_multi(tmpG, tmpX) or return

		/*
			First compute 1/G mod P.
		*/
		internal_invmod(tmpG, G, P) or return

		/*
			now get |X|.
		*/
		internal_abs(tmpX, X) or return
	...
	*/
	can_reduce_2k_l := _private_int_reduce_is_2k_l(P) or return
	if can_reduce_2k_l {
	...
	*/
	if dr == 0 {
		reduce_is_2k := _private_int_reduce_is_2k(P) or return
...
internal_int_kronecker :: proc(a, p: ^Int, allocator := context.allocator) -> (kronecker: int, err: Error) {
	...
	internal_copy(a1, a) or return
	internal_copy(p1, p) or return

	v := internal_count_lsb(p1) or return
	internal_shr(p1, p1, v) or return
	...
	for {
		...
		v = internal_count_lsb(a1) or return
		internal_shr(a1, a1, v) or return
		...
		internal_copy(r, a1) or return
		r.sign = .Zero_or_Positive

		internal_mod(a1, p1, r) or return
		internal_copy(p1, r)    or return
	}
	return
}
...
internal_int_prime_miller_rabin :: proc(a, b: ^Int, allocator := context.allocator) -> (probably_prime: bool, err: Error) {
	...
	*/
	...
	/*
		Get `n1` = `a` - 1.
	*/
	internal_copy(n1, a) or return
	internal_sub(n1, n1, 1) or return

	/*
		Set `2`**`s` * `r` = `n1`
	*/
	internal_copy(r, n1) or return

	/*
		Count the number of least significant bits which are zero.
	*/
	s := internal_count_lsb(r) or return

	/*
		Now divide `n` - 1 by `2`**`s`.
	*/
	internal_shr(r, r, s) or return

	/*
		Compute `y` = `b`**`r` mod `a`.
	*/
	internal_int_exponent_mod(y, b, r, a) or return
	...
	if !internal_eq(y, 1) && !internal_eq(y, n1) {
		...
		*/
		for j <= (s - 1) && !internal_eq(y, n1) {
			internal_sqrmod(y, y, a) or return
...
internal_int_is_prime :: proc(a: ^Int, miller_rabin_trials := int(-1), miller_rabin_only := USE_MILLER_RABIN_ONLY, r: ^rnd.Rand = nil, allocator := context.allocator) -> (is_prime: bool, err: Error) {
	...
	if internal_int_is_square(a) or return { return }
	...
	if internal_int_prime_is_divisible(a) or return { return }

	// Run the Miller-Rabin test with base 2 for the BPSW test.
	internal_set(b, 2) or return
	if !(internal_int_prime_miller_rabin(a, b) or return) { return }
	...
	if !miller_rabin_only {
		if miller_rabin_trials >= 0 {
			when MATH_BIG_USE_FROBENIUS_TEST {
				if !(internal_int_prime_frobenius_underwood(a) or return) { return }
			} else {
				if !(internal_int_prime_strong_lucas_selfridge(a) or return) { return }
			}
		}
	}
	...
	if miller_rabin_trials < 0 {
		...
		atoi(b, "437ae92817f9fc85b7e5", 16) or return
		if internal_lt(a, b) {
			p_max = 12
		} else {
			/* 0x2be6951adc5b22410a5fd = 3_317_044_064_679_887_385_961_981 */
			atoi(b, "2be6951adc5b22410a5fd", 16) or return
			if internal_lt(a, b) {
				p_max = 13
			} else {
		...
		*/
		for ix := 0; ix < miller_rabin_trials; ix += 1 {
			...
			internal_random(b, _DIGIT_TYPE_BITS, r) or return
...
internal_int_prime_frobenius_underwood :: proc(N: ^Int, allocator := context.allocator) -> (result: bool, err: Error) {
	...
	internal_init_multi(T1z, T2z, Np1z, sz, tz) or return
	...
	frob: for a = 0; a < _FROBENIUS_UNDERWOOD_A; a += 1 {
		...
		j := internal_int_kronecker(T1z, N) or return
	...
	internal_int_gcd(T1z, T1z, N) or return
	...
	ap2 = a + 2
	internal_add(Np1z, N, 1) or return

	internal_set(sz, 1) or return
	internal_set(tz, 2) or return
	...
	for i := internal_count_bits(Np1z) - 2; i >= 0; i -= 1 {
		...
		internal_int_shl1(T2z, tz) or return

		// a = 0 at about 50% of the cases (non-square and odd input)
		if a != 0 {
			internal_mul(T1z, sz, DIGIT(a)) or return
			internal_add(T2z, T2z, T1z) or return
		}

		internal_mul(T1z, T2z, sz) or return
		internal_sub(T2z, tz, sz) or return
		internal_add(sz, sz, tz) or return
		internal_mul(tz, sz, T2z) or return
		internal_mod(tz, tz, N) or return
		internal_mod(sz, T1z, N) or return
		...
		if bit, _ := internal_int_bitfield_extract_bool(Np1z, i); bit {
			...
			// sz   = temp
			if a == 0 {
				internal_int_shl1(T1z, sz) or return
			} else {
				internal_mul(T1z, sz, DIGIT(ap2)) or return
			}
			internal_add(T1z, T1z, tz) or return
			internal_int_shl1(T2z, tz) or return
	...
	internal_set(T1z, u32((2 * a) + 5)) or return
	internal_mod(T1z, T1z, N) or return
...
internal_int_prime_strong_lucas_selfridge :: proc(a: ^Int, allocator := context.allocator) -> (lucas_selfridge: bool, err: Error) {
	...
	*/
	internal_init_multi(Dz, gcd, Np1, Uz, Vz, U2mz, V2mz, Qmz, Q2mz, Qkdz, T1z, T2z, T3z, T4z, Q2kdz) or return
	...
	for {
		...
		internal_set(Dz, D) or return
		internal_int_gcd(gcd, a, Dz) or return
		...
		*/
		...
		j := internal_int_kronecker(Dz, a) or return
		if j == -1 { break }
	...
	*/
	internal_add(Np1, a, 1) or return
	s := internal_count_lsb(Np1) or return
	...
	/*
		...
		any leftovers.
	*/
	internal_int_shr(Dz, Np1, s) or return
	...
	*/
	internal_set(Uz,   1) or return
	internal_set(Vz,   1) or return //	P := 1; /* Selfridge's choice */
	internal_set(U2mz, 1) or return
	internal_set(V2mz, 1) or return //	P := 1; /* Selfridge's choice */
	internal_set(Qmz,  Q) or return

	internal_int_shl1(Q2mz, Qmz) or return

	/*
		Initializes calculation of Q^d.
	*/
	internal_set(Qkdz, Q) or return
	...
	for u := 1; u < Nbits; u += 1 { /* zero bit off, already accounted for */
		/*
			...
			V_2m = V_m*V_m - 2*Q^m
		*/
		internal_mul(U2mz, U2mz, V2mz) or return
		internal_mod(U2mz, U2mz, a) or return
		internal_sqr(V2mz, V2mz) or return
		internal_sub(V2mz, V2mz, Q2mz) or return
		internal_mod(V2mz, V2mz, a) or return

		/*
			Must calculate powers of Q for use in V_2m, also for Q^d later.
		*/
		internal_sqr(Qmz, Qmz) or return

		/* Prevents overflow. Still necessary without a fixed prealloc'd mem.? */
		internal_mod(Qmz, Qmz, a) or return
		internal_int_shl1(Q2mz, Qmz) or return

		if internal_int_bitfield_extract_bool(Dz, u) or return {
			/*
			...
			*/
			internal_mul(T1z, U2mz, Vz) or return
			internal_mul(T2z, Uz, V2mz) or return
			internal_mul(T3z, V2mz, Vz) or return
			internal_mul(T4z, U2mz, Uz) or return
			internal_mul(T4z, T4z,  Ds) or return

			internal_add(Uz,  T1z, T2z) or return

			if internal_is_odd(Uz) {
				internal_add(Uz, Uz, a) or return
			}
			...
			*/
			...
			internal_int_shr1(Uz, Uz) or return
			if internal_is_negative(Uz) && oddness {
				internal_sub(Uz, Uz, 1) or return
			}
			internal_add(Vz, T3z, T4z) or return
			if internal_is_odd(Vz) {
				internal_add(Vz, Vz, a) or return
			}

			oddness  = internal_is_odd(Vz)
			internal_int_shr1(Vz, Vz) or return
			if internal_is_negative(Vz) && oddness {
				internal_sub(Vz, Vz, 1) or return
			}
			internal_mod(Uz, Uz, a) or return
			internal_mod(Vz, Vz, a) or return

			/* Calculating Q^d for later use */
			internal_mul(Qkdz, Qkdz, Qmz) or return
			internal_mod(Qkdz, Qkdz, a) or return
		}
	}
	...
	*/
	...
	internal_int_shr1(Q2kdz, Qkdz) or return

	for r := 1; r < s; r += 1 {
		internal_sqr(Vz, Vz) or return
		internal_sub(Vz, Vz, Q2kdz) or return
		internal_mod(Vz, Vz, a) or return
		...
		if r < (s - 1) {
			internal_sqr(Qkdz, Qkdz) or return
			internal_mod(Qkdz, Qkdz, a) or return
			internal_int_shl1(Q2kdz, Qkdz) or return
		}
	}
	return false, nil
}
...
internal_prime_fermat :: proc(a, b: ^Int, allocator := context.allocator) -> (fermat: bool, err: Error) {
	...
	*/
	...
	/*
		Compute `t` = `b`**`a` mod `a`
	*/
	internal_int_exponent_mod(t, b, a, a) or return
...
internal_int_sqrtmod_prime :: proc(res, n, prime: ^Int, allocator := context.allocator) -> (err: Error) {
	...
	*/
	...
	legendre := internal_int_kronecker(n, prime)                     or return
	...
	*/
	...
	internal_init_multi(t1, C, Q, Z, T, R, two)                      or return
	...
	*/
	if prime.digit[0] & 3 == 3 {
		internal_add(t1, prime, 1)                                   or return
		internal_shr(t1, t1, 2)                                      or return
		internal_int_exponent_mod(res, n, t1, prime)                 or return
		return
	}
	...
	/*
		...
		Q = prime - 1
	*/
	internal_copy(Q, prime)                                          or return
	internal_sub(Q, Q, 1)                                            or return
	...
	*/
	...
	for internal_is_even(Q) {
		/*
			Q = Q / 2
		*/
		internal_int_shr1(Q, Q)                                      or return
		/*
			S = S + 1
		*/
		S += 1
	...
	/*
		...
		Z = 2.
	*/
	internal_set(Z, 2)                                               or return

	for {
		legendre = internal_int_kronecker(Z, prime)                  or return
		...
		*/
		...
		/*
			Z = Z + 1
		*/
		internal_add(Z, Z, 1)                                        or return
	}

	/*
		C = Z ^ Q mod prime
	*/
	internal_int_exponent_mod(C, Z, Q, prime)                        or return

	/*
		t1 = (Q + 1) / 2
	*/
	internal_add(t1, Q, 1)                                           or return
	internal_int_shr1(t1, t1)                                        or return

	/*
		R = n ^ ((Q + 1) / 2) mod prime
	*/
	internal_int_exponent_mod(R, n, t1, prime)                       or return

	/*
		T = n ^ Q mod prime
	*/
	internal_int_exponent_mod(T, n, Q, prime)                        or return
	...
	for {
		...
		for {
			...
			*/
			...
			internal_int_exponent_mod(t1, t1, two, prime)            or return

			i += 1
		}

		if i == 0 {
			internal_copy(res, R)                                    or return
		}

		/*
			t1 = 2 ^ (M - i - 1)
		*/
		internal_set(t1, M - i - 1)                                  or return
		internal_int_exponent_mod(t1, two, t1, prime)                or return

		/*
			t1 = C ^ (2 ^ (M - i - 1)) mod prime
		*/
		internal_int_exponent_mod(t1, C, t1, prime)                  or return

		/*
			C = (t1 * t1) mod prime
		*/
		internal_sqrmod(C, t1, prime)                                or return

		/*
			R = (R * t1) mod prime
		*/
		internal_mulmod(R, R, t1, prime)                             or return

		/*
			T = (T * C) mod prime
		*/
		mulmod(T, T, C, prime)                                       or return
...
internal_int_prime_next_prime :: proc(a: ^Int, trials: int, bbs_style: bool, allocator := context.allocator) -> (err: Error) {
	...
	*/
	if bbs_style {
		...
		*/
		if a.digit[0] & 3 != 3 {
			internal_sub(a, a, (a.digit[0] & 3) + 1) or return
		}
	} else {
		if internal_is_even(a) {
			/*
	...
	*/
	for x := 1; x < _PRIME_TAB_SIZE; x += 1 {
		res_tab = internal_mod(a, _private_prime_table[x]) or return
	}
...
internal_random_prime :: proc(a: ^Int, size_in_bits: int, trials: int, flags := Primality_Flags{}, r: ^rnd.Rand = nil, allocator := context.allocator) -> (err: Error) {
	...
	*/
	...
	for {
		...
		internal_int_random(a, size_in_bits)                         or return
		...
		*/
		...
		if .Second_MSB_On in flags {
			internal_int_bitfield_set_single(a, size_in_bits - 2) or return
		}

		/*
			Is it prime?
		*/
		res = internal_int_is_prime(a, trials)                       or return
...
internal_int_extended_euclidean :: proc(a, b, U1, U2, U3: ^Int, allocator := context.allocator) -> (err: Error) {
	...
	internal_init_multi(u1, u2, u3, v1, v2, v3, t1, t2, t3, q, tmp)  or return

	/*
		Initialize, (u1, u2, u3) = (1, 0, a).
	*/
	internal_set(u1, 1)                                              or return
	internal_set(u3, a)                                              or return

	/*
		Initialize, (v1, v2, v3) = (0, 1, b).
	*/
	internal_set(v2, 1)                                              or return
	internal_set(v3, b)                                              or return
	...
	*/
	for !internal_is_zero(v3) {
		/*
			q = u3 / v3
		*/
		internal_div(q, u3, v3)                                      or return

		/*
			(t1, t2, t3) = (u1, u2, u3) - (v1, v2, v3)q
		*/
		internal_mul(tmp, v1, q)                                     or return
		internal_sub( t1, u1, tmp)                                   or return

		internal_mul(tmp, v2, q)                                     or return
		internal_sub( t2, u2, tmp)                                   or return

		internal_mul(tmp, v3, q)                                     or return
		internal_sub( t3, u3, tmp)                                   or return

		/*
			(u1, u2, u3) = (v1, v2, v3)
		*/
		internal_set(u1, v1)                                         or return
		internal_set(u2, v2)                                         or return
		internal_set(u3, v3)                                         or return

		/*
			(v1, v2, v3) = (t1, t2, t3)
		*/
		internal_set(v1, t1)                                         or return
		internal_set(v2, t2)                                         or return
		internal_set(v3, t3)                                         or return
	}
	...
	*/
	if internal_is_negative(u3) {
		internal_neg(u1, u1)                                         or return
		internal_neg(u2, u2)                                         or return
		internal_neg(u3, u3)                                         or return
	}
