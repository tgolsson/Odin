...
gb_internal ExprKind check_or_branch_expr(CheckerContext *c, Operand *o, Ast *node, Type *type_hint) {
	...
	case Token_break:    name = str_lit("or break"); break;
	case Token_continue: name = str_lit("or continue"); break;
	}
	Operand x = {};
	...
	case Token_break:
		if ((c->stmt_flags & Stmt_BreakAllowed) == 0 && label == nullptr) {
			error(be->token, "'or break' only allowed in non-inline loops or 'switch' statements");
		}
		break;
	case Token_continue:
		if ((c->stmt_flags & Stmt_ContinueAllowed) == 0 && label == nullptr) {
			error(be->token, "'or continue' only allowed in non-inline loops");
		}
		break;
	}
	...
	if (label != nullptr) {
		...
		case Ast_IfStmt:
		case Ast_SwitchStmt:
			if (be->token.kind != Token_break) {
				error(label, "Label '%.*s' can only be used with 'break'", LIT(e->token.string));
			}
			break;
		case Ast_RangeStmt:
		case Ast_ForStmt:
			if ((be->token.kind != Token_break) && (be->token.kind != Token_continue)) {
				error(label, "Label '%.*s' can only be used with 'break' and 'continue'", LIT(e->token.string));
			}
			break;
...
gb_internal gbString write_expr_to_string(gbString str, Ast *node, bool shorthand) {
	...
	case_ast_node(oe, OrBranchExpr, node);
		str = write_expr_to_string(str, oe->expr, shorthand);
		str = gb_string_append_fmt(str, "or %.*s", LIT(oe->token.string));
		if (oe->label) {
			str = gb_string_append_rune(str, ' ');
